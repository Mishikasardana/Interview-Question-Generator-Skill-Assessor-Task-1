"""
Configuration loader for the JD Parsing Module.
...
Purpose:
    ...
    environment variables as ``resume_processing`` and ``question_generation``
...
Outputs:
    Immutable configuration values consumed by ``jd_parser.py``.

Example usage:
    >>> from jd_parsing.config import get_glm_api_key
...
def get_glm_api_key() -> str:
    ...
    Raises:
        JDParsingError: If neither is set.
    """
